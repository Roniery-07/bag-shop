...
export class ProductRepositoryPrisma implements ProductGateway {
  ...
  public async list(search: string): Promise<Product[]> {
    let products;
    if (search) {
      products = await this.prismaClient.product.findMany({
        where: {
          name: {
            search: search,
          },
  ...
  public async getAvailableStockQuantity(productId: string): Promise<number> {
    ...
    if (res?.quantity == null) throw new Error('Product quantity is null!');
